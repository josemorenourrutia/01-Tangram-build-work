...
export function createNumbers ( length, step = 1, positionY = 0.7, color = '#555555' ) {
	...
	// const numberMesh = createNumberPlane( i, numbersTexture, length, 0.3 );
	// numberMesh.position.set( -length / 2 + i * cmSpacing, width / 2 + 0.25, 0.02 );
	// group.add( numberMesh );

	const cmSpacing = 1, width = 1.5;
	const numbers = new THREE.Group();
	for ( let i = 0; i <= length; i++ ) {
		// const plane = makeTextPlane( i.toString(), 160, color );
		// plane.position.set( i, positionY, 0.05 );
		const numberMesh = createNumberPlane( i, numbersTexture, 30, 0.6 );
		// numberMesh.position.set( -length / 2 + i * cmSpacing, width / 2 + 0.25, 0.02 );
		numberMesh.position.set( i, positionY, 0.05 );
		numberMesh.scale.set( 1, -1, 1 )
...
export function createNumbers1 ( length, step = 1, positionY = 0.7, color = '#555555' ) {
	const numbers = new THREE.Group();
	for ( let i = 0; i <= length; i++ ) {
		const plane = makeTextPlane( i.toString(), 160, color );
		plane.position.set( i, positionY, 0.05 );
...
export function createRuler ( lengthCm = 20, widthCm = 2, margin = 0.5 ) {
	...
	group.add( ruler, marks, numbers );
	return group;
}

// --- Escuadra 45°-45°-90° ---
export function createSetSquare ( size = 10, margin = 0.5 ) {
	const shape = new THREE.Shape();
	shape.moveTo( 0, 0 );
	shape.lineTo( size, 0 );
	shape.lineTo( 0, -size );
	...
	const geometry = new THREE.ShapeGeometry( shape );
	const material = new THREE.MeshBasicMaterial( { color: 0xf5deb3, side: THREE.DoubleSide } );
	...
	const group = new THREE.Group();
	group.add( triangle, marksX, numbersX );//, marksY, numbersY );
	return group;
}
